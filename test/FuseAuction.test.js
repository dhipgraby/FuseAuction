...
describe("FuseAuction", function () {
    ...
    describe("Auction creation", function () {
        it("Should create a native token auction", async function () {
            ...
            const tx = await fuseAuction.connect(owner).createNativeAuction(1, 60 * 60, ethers.utils.parseEther("0.1"), nftMock.address);
            await tx.wait();
        ...
        it("Should create an ERC20 token auction", async function () {
            ...
            const tx = await fuseAuction.connect(owner).createERC20Auction(2, 60 * 60, 100, nftMock.address, erc20Mock.address);
            await tx.wait();
